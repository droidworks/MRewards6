...
<div class="action_info">
    ...
    <div class="action_data">
        ...
        {if $item.action_mode == 'update'}
            <span class="action_desc"><a href="{$jamroom_url}/{$item.action_data.profile_url}/{$murl}/{$item.action_data._item_id}/{$item.action_data.page_title_url}">{jrCore_lang module="jrPage" id="21" default="Updated a Page"}.</a></span>
        {else}
            <a href="{$jamroom_url}/{$item.action_data.profile_url}/{$murl}/{$item.action_data._item_id}/{$item.action_data.page_title_url}" title="{$item.action_data.page_title|jrCore_entity_string}">{jrCore_lang module="jrPage" id="18" default="Created a new Page"}.</a>
        {/if}

        <br><span class="action_time">{$item.action_data._created|jrCore_date_format:"relative"}</span>
