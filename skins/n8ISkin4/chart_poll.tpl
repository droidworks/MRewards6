...
{if isset($_items)}
    {foreach from=$_items item="item"}
        {assign var="_data" value=$item.poll_data|json_decode:true}
        <div class="list_item">
           <div class="wrap">
               ...
               <div class="data clearfix">
                   <span>{$item.poll_comment_count|jrCore_number_format} {jrCore_lang skin=$_conf.jrCore_active_skin id="109" default="Comments"}</span>
                   <span>{$item.poll_like_count|jrCore_number_format} {jrCore_lang skin=$_conf.jrCore_active_skin id="110" default="Likes"}</span>
               </div>
           </div>
        </div>
    {/foreach}
