            ...
            <table style="width:100%;">
                <tr>
                    ...
                    {if isset($info.prev_page) && $info.prev_page > 0}
                        <a onClick="jrLoad('#imagebrowse','{$info.page_base_url}/p={$info.prev_page}');$('html, body').animate({ scrollTop: $('#imgbro').offset().top -100 }, 'slow');">&laquo; Previous</a>
                    {else}
                    ...
                    <td class="p10" style="width:50%;text-align:center;">
                        ...
                        <form id="form" name="form" method="post" action="_self">
                            <select name="pagenum" class="form_select" style="width:60px;" onchange="var sel=this.form.pagenum.options[this.form.pagenum.selectedIndex].value;jrLoad('#imagebrowse','{$info.page_base_url}/p=' +sel);$('html, body').animate({ scrollTop: $('#imgbro').offset().top -100 }, 'slow');">
                    ...
                    {if isset($info.next_page) && $info.next_page > 1}
                        <a onClick="jrLoad('#imagebrowse','{$info.page_base_url}/p={$info.next_page}');$('html, body').animate({ scrollTop: $('#imgbro').offset().top -100 }, 'slow');">Next &raquo;</a>
                    {else}
...
{if jrCore_module_is_active('jrFlickr')}
    {assign var="browse_pb" value="2"}
{else}
    {assign var="browse_pb" value="5"}
{/if}

{jrCore_list module="jrGallery" order="_item_id random" template=$index_image_row pagebreak=$browse_pb page=$_post.p require_image_width=$_conf.jrPhoto_min_width}
