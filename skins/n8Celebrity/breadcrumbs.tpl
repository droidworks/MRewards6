...
{elseif $nav_mode == "jrBlog"}
    ...
    {if $page == 'detail' || $page == 'group'}
        <a href="{$jamroom_url}/{$profile_url}/{$nav_murl}/category">{jrCore_lang skin=$_conf.jrCore_active_skin id="22" default="Categories"}</a>
    {/if}
...
{elseif $nav_mode == "jrEvent"}
    ...
    {if $page == 'detail' || $page == 'group'}
        <a href="{$jamroom_url}/{$profile_url}/{$nav_murl}/calendar">{jrCore_lang skin=$_conf.jrCore_active_skin id="30" default="Calendar"}</a>
    {/if}
...
{elseif $nav_mode == "jrVimeo"}
    ...
    {if strlen($item.vimeo_title) > 0 && $page == 'detail'}
        <a href="{$jamroom_url}/{$profile_url}/{$nav_murl}/{$item._item_id}/{$item.vimeo_title_url}">{$item.vimeo_title}</a>
    {/if}
...
{elseif $nav_mode == "jrPage"}
    <a href="{$jamroom_url}/{$profile_url}/{$nav_murl}">{jrCore_lang module="jrPage" id="19" default="Item Bundles"}</a>
...
{elseif $nav_mode == "jrSoundCloud"}
    {if jrCore_module_is_active('jrCombinedAudio') && $item.quota_jrCombinedAudio_allowed == 'on'}
        <a href="{$jamroom_url}/{$profile_url}/{jrCore_module_url module="jrCombinedAudio"}">{jrCore_lang module="jrCombinedAudio" id=1 default="Audio"}</a>
    {else}
        <a href="{$jamroom_url}/{$item.profile_url}/{$nav_murl}">{jrCore_lang module="jrSoundCloud" id="53" default="SoundCloud"}</a>
    {/if}
    {if strlen($item.soundcloud_title) > 0 && $page == 'detail'}
        <a href="{$jamroom_url}/{$profile_url}/{$nav_murl}/{$item._item_id}/{$item.soundcloud_title_url}">{$item.soundcloud_title}</a>
    {/if}
{elseif $nav_mode == "jrYouTube"}
    ...
    {else}
        <a href="{$jamroom_url}/{$item.profile_url}/{$nav_murl}">{jrCore_lang module="jrYouTube" id="40" default="YouTube"}</a>
    {/if}
    {if strlen($item.youtube_title) > 0 && $page == 'detail'}
        <a href="{$jamroom_url}/{$profile_url}/{$nav_murl}/{$item._item_id}/{$item.youtube_title_url}">{$item.youtube_title}</a>
    {/if}
{elseif $nav_mode == "jrGallery"}
    ...
    {/if}
    <a href="{$jamroom_url}/{$item.profile_url}/{$nav_murl}">{$heading}</a>

    {if strlen($item.gallery_title) > 0}
        <a href="{$jamroom_url}/{$item.profile_url}/{$nav_murl}/{$item.gallery_title_url}/all">{$item.gallery_title}</a>
    {/if}
...
{elseif $nav_mode == 'jrForum'}
    <a href="{$jamroom_url}/{$profile_url}/{$nav_murl}">{jrCore_lang skin=$_conf.jrCore_active_skin id="1" default="Playlist"}</a>
{elseif $nav_mode == 'jrDoc'}
   <a href="{$jamroom_url}/{$profile_url}/{$nav_murl}">{jrCore_lang module="jrDocs" id="53" default="Documentation"}</a>
...
{elseif $nav_mode == 'jrGroupDiscuss'}
    ...
    {if isset($item.discuss_group_id)}
        ...
        <a href="{$jamroom_url}/{$profile_url}/{$nav_murl}/group_id={$item.discuss_group_id}">{jrCore_lang module="jrGroupDiscuss" id="1" default="Discussions"}</a>
    {else}
        <a href="{$jamroom_url}/{$profile_url}/{$gurl}/{$item._item_id}/{$item.group_title_url}">{$item.group_title}</a>
        <a href="{$jamroom_url}/{$profile_url}/{$nav_murl}/group_id={$item._item_id}">{jrCore_lang module="jrGroupDiscuss" id="1" default="Discussions"}</a>
    {/if}
...
{elseif $nav_mode == 'jrGroupPage'}
    ...
    {if isset($item.npage_group_id)}
        ...
        <a href="{$jamroom_url}/{$profile_url}/{$nav_murl}/group_id={$item.npage_group_id}">{jrCore_lang module="jrGroupPage" id="10" default="Group Page"}</a>
    {else}
        <a href="{$jamroom_url}/{$profile_url}/{$gurl}/{$item._item_id}/{$item.group_title_url}">{$item.group_title}</a>
        <a href="{$jamroom_url}/{$profile_url}/{$nav_murl}/group_id={$item._item_id}">{jrCore_lang module="jrGroupPage" id="10" default="Group Page"}</a>
    {/if}
