...
<div class="page_nav clearfix">
    <div class="breadcrumbs">
        {n8BeatSlinger_breadcrumbs nav_mode="jrGroup" profile_url=$item.profile_url profile_name=$item.profile_name page="detail" item=$item}
    </div>
...
<div class="row">
    <div class="col9">
        <div class="box">
            <ul class="head_tab">
                <li id="group_tab">
                    <a href="#" title="{jrCore_lang module="jrGroupDiscuss" id=20 default="Discussion Forum"}"></a>
                </li>
            </ul>
            <div class="box_body">
        ...
        <div class="box">
            <ul class="head_tab">
                <li id="forum_tab">
                    <a href="#" title="{jrCore_lang module="jrGroup" id=1 default="Group"}"></a>
                </li>
                <div class="action_buttons">
                    ...
                    {if jrProfile_is_profile_owner($item._profile_id) || jrGroup_get_user_config('jrGroupDiscuss', 'allowed', $item, $_user._user_id) == 'on'}
                        ...
                        <a href="{$jamroom_url}/{$ndurl}/create/group_id={$item._item_id}"
                           title="{$ttl|jrCore_entity_string}">{jrCore_icon icon="plus" size=16}</a>
                    {/if}
                </div>
            </ul>
            <div class="box_body">
                    ...
                    <div class="action_feedback" style="padding: 0">
                        {if jrGroup_member_has_access($item)}
                            {if !jrCore_module_is_active('jrComment') || !isset($item.group_wall) || $item.group_wall == 'off'}
                                {n8BeatSlinger_feedback_buttons module="jrGroup" comment=false item=$item}
                                {jrCore_item_detail_features module="jrGroup" item=$item exclude="jrComment~item_comments,jrDisqus~disqus_comment"}
                            {else}
                                {n8BeatSlinger_feedback_buttons module="jrGroup" item=$item}
    ...
    <div class="col3 last">
        <div class="box" style="padding-left:1em">
            <ul class="head_tab">
                <li id="followers_tab">
                    <a href="#" title="{jrCore_lang module="jrGroup" id=13 default="Members"}"></a>
                </li>
            </ul>
            <div class="box_body">
                <div class="wrap">
                    ...
                    {if jrProfile_is_profile_owner($item._profile_id) || jrGroup_get_user_config('jrGroupPage', 'allowed', $item, $_user._user_id) == 'on'}
                        <div class="action_buttons">
                            ...
                            <a href="{$jamroom_url}/{$npurl}/create/group_id={$item._item_id}"
                               title="{$pttl|jrCore_entity_string}">{jrCore_icon icon="plus" size=16}</a>
                        </div>
                    {/if}
                </ul>
                <div class="box_body">
