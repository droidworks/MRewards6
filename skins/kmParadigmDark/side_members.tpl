...
{if isset($_conf.kmParadigmDark_show_radio) && $_conf.kmParadigmDark_show_radio == 'on'}
    {if isset($_conf.kmParadigmDark_radio_title) && strlen($_conf.kmParadigmDark_radio_title) > 0}
        {jrCore_list module="jrPlaylist" profile_id="0" order_by="_created desc" search1="playlist_title = `$_conf.kmParadigmDark_radio_title`" limit="1" template="index_radio.tpl"}
        {else}
        {if jrUser_is_logged_in()}
            ...
            <div class="body_2b normal p20 mb20">
                Admin Note:&nbsp;<a href="{$jamroom_url}/core/skin_admin/global/skin=kmParadigmDark">Settings</a> <b>"Radio Title"</b> is not set!
            </div>
            {/if}
        {/if}
    {/if}
...
<div class="page mb20 pt10">
    ...
    {if isset($_conf.kmParadigmDark_featured_member) && $_conf.kmParadigmDark_featured_member > 0}
        {jrCore_list module="jrProfile" search="_profile_id = `$_conf.kmParadigmDark_featured_member`" template=$feat_mem_today}
    {else}
        {if isset($_conf.kmParadigmDark_require_images) && $_conf.kmParadigmDark_require_images == 'on'}
            {if isset($_conf.kmParadigmDark_member_quota) && $_conf.kmParadigmDark_member_quota > 0}
                {jrCore_list module="jrProfile" order_by="_item_id random" limit="1" search1="profile_active = 1" quota_id=$_conf.kmParadigmDark_member_quota template=$feat_mem_today require_image="profile_image"}
            {else}
        ...
        {else}
            {if isset($_conf.kmParadigmDark_member_quota) && $_conf.kmParadigmDark_member_quota > 0}
                {jrCore_list module="jrProfile" order_by="_item_id random" limit="1" search1="profile_active = 1" quota_id=$_conf.kmParadigmDark_member_quota template=$feat_mem_today}
            {else}
...
{if isset($_conf.kmParadigmDark_show_tv) && $_conf.kmParadigmDark_show_tv == 'on'}
    {if isset($_conf.kmParadigmDark_tv_title) && strlen($_conf.kmParadigmDark_tv_title) > 0}
        {jrCore_list module="jrPlaylist" profile_id="0" order_by="_created desc" search1="playlist_title = `$_conf.kmParadigmDark_tv_title`" tpl_dir="kmParadigmDark" template="index_channel.tpl"}
        {else}
        {if jrUser_is_logged_in()}
            {if jrUser_is_master()}
