...
{/if}
            ...
            <span class="comment_text">
                ...
                {if jrUser_is_logged_in()}
                    <div id="bc{$item._item_id}" style="position:absolute;top:5px;right:0;display:none">
                        <script>$(function () {
                ...
                {jrCore_get_uploaded_files module="jrComment" item=$item field="comment_file"}
                    ...
                    <div>
                        {if jrUser_is_logged_in() && $_conf.jrComment_threading == 'on'}
                            ...
                            <a href="#" onclick="n8Maestro4_reply_to({$item._item_id}, '{$item.user_name|addslashes}','#{$item.comment_module}_{$item.comment_item_id}_comments' )">Reply</a>
                            &nbsp;&nbsp;
                        {/if}
