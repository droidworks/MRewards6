...
{if isset($item)}
    ...
    {$item = jrCore_db_get_item('jrPlaylist', $item.action_item_id)}
{else}
    {$item = $_items[0]}
{/if}
...
<div class="action_info">
    ...
    <div class="action_data">
        ...
        {if $item_mode == 'create'}
            <span class="action_desc"><a href="{$jamroom_url}/{$item.profile_url}/{$murl}/{$item._item_id}/{$item.playlist_title_url}" title="{$item.playlist_title|jrCore_entity_string}">
                    {jrCore_lang module="jrPlaylist" id="1" default="Created a new Playlist"}.</a></span><br>
        {else}
            <a href="{$jamroom_url}/{$item.profile_url}/{$murl}/{$item._item_id}/{$item.playlist_title_url}" title="{$item.playlist_title|jrCore_entity_string}">
...
<div class="media">
    {if isset($item) && isset($item._item_id)}
        {jrCore_media_player module="jrPlaylist" type="n8Player_playlist_action_player" item=$item autoplay=false}
    {else}
        Item deleted.
    {/if}
</div>
