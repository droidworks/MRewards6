...
{if $item.action_mode == 'create'}
    {if isset($item.action_data.audio_active) && $item.action_data.audio_active == 'off' && isset($item.action_data.quota_jrAudio_audio_conversions) && $item.action_data.quota_jrAudio_audio_conversions == 'on'}
        <p class="center action_item_text">{jrCore_lang module="pmPrivateAudio" id=40 default="This audio file is currently being processed and will appear here when complete."}</p>
    {else}
        <span class="action_item_desc">{jrCore_lang module="pmPrivateAudio" id=68 default="Created a new Audio File"}:</span><br>
...
{else}
    {if isset($item.action_data.audio_active) && $item.action_data.audio_active == 'off' && isset($item.action_data.quota_jrAudio_audio_conversions) && $item.action_data.quota_jrAudio_audio_conversions == 'on'}
        <p class="center p5">{jrCore_lang module="pmPrivateAudio" id=40 default="This audio file is currently being processed and will appear here when complete."}</p>
    {else}
