...
<div class="block">
    ...
    {capture name="forum_posts" assign="forum_posts"}
    <div class="block_content">
        ...
        {foreach $_items as $_itm}
            ...
            <div id="p{$_itm._item_id}" class="item" style="position:relative">
                    ...
                    <div style="display:table-row">
                        {if !jrCore_is_mobile_device()}
                            <div class="forum_post_image">
                                {jrCore_module_function function="jrImage_display" module="jrUser" type="user_image" item_id=$_itm._user_id size="icon" crop="portrait" alt=$_itm.user_name class="action_item_user_img iloutline" width=96 height=96}<br>
                                <span class="normal"><a href="{$jamroom_url}/{$_itm.profile_url}">@{$_itm.profile_url}</a><br>{$_itm._created|jrCore_format_time:false:"relative"}<br><a href="{$jamroom_url}/{$murl}/activity/{$_itm._profile_id}/{$_itm.profile_url}">{$_itm.user_jrForum_item_count|default:0|number_format} {jrCore_lang module="jrForum" id="35" default="posts"}</a></span>
                            </div>
                        {/if}
                        <div class="forum_post_text">

                            {if jrCore_is_mobile_device()}
                                <span class="normal"><a href="{$jamroom_url}/{$_itm.profile_url}">@{$_itm.profile_url}</a> &bull; {$_itm._created|jrCore_format_time:false:"relative"} &bull; <a href="{$jamroom_url}/{$murl}/activity/{$_itm._profile_id}/{$_itm.profile_url}">{$_itm.user_jrForum_item_count|default:0|number_format} {jrCore_lang module="jrForum" id="35" default="posts"}:</a></span><br><br>
                            {/if}
                ...
                {if jrUser_is_logged_in() && (jrUser_is_admin() || $topic.forum_locked != 'on')}
                    <div id="m{$_itm._item_id}" class="forum_actions">

                        <script type="text/javascript">$(function() { var mid = $('#m{$_itm._item_id}'); $('#p{$_itm._item_id}').hover(function() { mid.fadeIn('fast'); }, function() { mid.fadeOut('fast'); });});</script>
                        ...
                        {if $_itm.user_can_edit && (jrProfile_is_profile_owner($_profile_id) || ($_itm._user_id == $_user._user_id && $_itm.forum_locked != 'on'))}
    ...
    {capture name="forum_form" assign="forum_form"}
        {* New entry form *}
        {if jrUser_is_admin() || (jrUser_is_logged_in() && $_user.quota_jrForum_can_post == 'on')}
            {if jrUser_is_admin() || $topic.forum_locked != 'on'}
                {if !jrUser_is_admin() && !jrProfile_is_profile_owner($topic.forum_profile_id) && $_conf.jrForum_allow_private == 'off' && $_user.profile_private != '1'}
                    ...
                    <div class="item" style="display:table">
                        <div style="display:table-row">
                            <div class="p5" style="display:table-cell;width:5%;vertical-align:top;">
                                {jrCore_module_function function="jrImage_display" module="jrUser" type="user_image" item_id=$_user._user_id size="icon" crop="portrait" alt=$item.user_name class="action_item_user_img iloutline" width=96 height=96}
                            </div>
                                ...
                                <div class="forum_new_post_form">
                                    ...
                                    <form id="cform" action="{$jamroom_url}/{$murl}/post_create_save" method="POST" onsubmit="jrForumPostResponse('#cform');return false">
                                        ...
                                        <div style="vertical-align:middle;">
                                            ...
                                            {if $_conf.jrForum_editor != 'on' || jrCore_is_mobile_device()}
                                                {* bb code help*}
                                            ...
                                            {if jrUser_get_profile_home_key('quota_jrForum_file_attachments') == 'on' && isset($_conf.jrForum_allowed_file_types)}
                                                <div class="jrforum_upload_attachment">
                                                    {jrCore_upload_button module="jrForum" field="forum_file" allowed=$_conf.jrForum_allowed_file_types multiple="true"}
                                                </div>
                                            {/if}
