...
<div class="block">
    ...
    <div class="title">
        ...
        <h1>{jrCore_lang module="jrForum" id="36" default="Forum"}{$hdr}</h1><br>
        ...
        {foreach $_items as $item}
            <div style="padding:12px 0">
                <div style="float:left;padding-right:12px;">
                    {jrCore_module_function function="jrImage_display" module="jrUser" type="user_image" item_id=$item._user_id size="small" crop="auto" width=40 alt=$item.user_name class="action_item_user_img iloutline" _v=$item._updated}
                </div>
                <div>
                    ...
                    {if $categories_enabled == 'on' && strlen($category_url) > 0}
                        {if $_conf.jrForum_direction == 'desc'}
                            <a href="{$jamroom_url}/{$profile_url}/{$murl}/{$category_url}/{$item._item_id}/{$item.forum_title_url}">
                        {elseif $_conf.jrForum_post_pagebreak > 0 && $item.forum_post_count > $_conf.jrForum_post_pagebreak}
                    ...
                    {else}
                       {if $_conf.jrForum_direction == 'desc'}
                            <a href="{$jamroom_url}/{$profile_url}/{$murl}/{$item._item_id}/{$item.forum_title_url}">
                       {elseif $_conf.jrForum_post_pagebreak > 0 && $item.forum_post_count > $_conf.jrForum_post_pagebreak}
                            {math equation="x / y" x=$item.forum_post_count y=$_conf.jrForum_post_pagebreak assign="last_page"}
                    ...
                    {/if}
                            {$item.forum_post_count} {jrCore_lang module="jrForum" id="35" default="posts"} {if $item.forum_updated_user_id > 0}<span style="display:inline-block" title="{jrCore_lang module="jrForum" id="34" default="update by"} {$item.forum_updated_user_name}, {$item.forum_updated|jrCore_format_time:false:"relative"}">{jrCore_module_function function="jrImage_display" module="jrUser" type="user_image" item_id=$item.forum_updated_user_id size="xsmall" crop="auto" width="24" alt=$item.forum_updated_user_name _v=$item._updated}</span>{/if}
                        </div></a>
                    ...
                    {if jrUser_is_logged_in()}
                        <span class="small_follow_container">
                            {if $item.forum_user_is_following == '1'}
                                {jrForum_follow_button icon="site-hilighted" item_id=$item._item_id size="12" show_result=false}
                            {else}
                                {jrForum_follow_button icon="site" item_id=$item._item_id size="12" show_result=false}
                            {/if}
                        </span>
                    {/if}
