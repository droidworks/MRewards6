...
<div id="site_news_div" class="mb20">
    ...
    {* SITE NEWS FUNCTION *}
    {if isset($_conf.kmParadigmDark_blog_profile) && $_conf.kmParadigmDark_blog_profile > 0}
        {jrCore_list module="jrBlog" order_by="_created desc" search1="_profile_id in `$_conf.kmParadigmDark_blog_profile`" search2="blog_category = news" template=$site_news_template pagebreak="1" page=$_post.p}
    {else}
...
<div class="page mb20 pt10">
    ...
    {jrCore_list module="jrProfile" order_by="_created desc" limit="1" search1="profile_active = 1" quota_id=$_conf.kmParadigmDark_member_quota template=$new_listener require_image="profile_image"}
</div>
...
<div class="page mb20 pt10">
    ...
    {/capture}
    {* NEW LISTENER FUNCTION *}
    {if isset($_conf.kmParadigmDark_todays_featured) && strlen($_conf.kmParadigmDark_todays_featured) > 0}
        {jrCore_list module="jrProfile" search="_profile_id = `$_conf.kmParadigmDark_todays_featured`" template=$featured_artist}
    {else}
        {jrCore_list module="jrProfile" order_by="profile_view_count numerical_desc" limit="1" search1="profile_active = 1" quota_id=$_conf.kmParadigmDark_artist_quota template=$featured_artist require_image="profile_image"}
    {/if}
</div>
...
{*{if isset($spt) && ($spt == 'music' || $spt == 'galleries' || $spt == 'home' || $spt == 'artist' || $spt == 'member' || $spt == 'profiles')}
    {if isset($_conf.kmParadigmDark_show_radio) && $_conf.kmParadigmDark_show_radio == 'on'}
        {if isset($_conf.kmParadigmDark_radio_title) && strlen($_conf.kmParadigmDark_radio_title) > 0}
            {jrCore_list module="jrPlaylist" profile_id="1" order_by="_created desc" search1="playlist_title = `$_conf.kmParadigmDark_radio_title`" limit="1" template="index_radio.tpl"}
        {else}
...
<script> if (typeof(fg_widgets)==="undefined") fg_widgets = new Array();fg_widgets.push("fgid_27fea69a118809e20536c1505");</script>
...
{* HOUSE CHANNEL *}
{*
    {if isset($_conf.kmParadigmDark_show_tv) && $_conf.kmParadigmDark_show_tv == 'on'}
        {if isset($_conf.kmParadigmDark_tv_title) && strlen($_conf.kmParadigmDark_tv_title) > 0}
            {jrCore_list module="jrPlaylist" profile_id="1" order_by="_created desc" search1="playlist_title = `$_conf.kmParadigmDark_tv_title`" template="index_channel.tpl"}
        {else}
            {if jrUser_is_logged_in()}
                {if jrUser_is_master()}
...
{if $_conf.kmParadigmDark_ads_off != 'on'}
    ...
    <div class="body_1 center mt20 mb20">
        ...
        {elseif isset($_conf.kmParadigmDark_side_ad) && strlen($_conf.kmParadigmDark_side_ad) > 0}
            {$_conf.kmParadigmDark_side_ad}
        {else}
